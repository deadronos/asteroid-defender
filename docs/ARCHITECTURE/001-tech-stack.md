...
- *Why:* It detaches game logic (position tracking, health deduction, target locking) from React's render cycle. Data mutates instantly in memory without triggering expensive React component tree updates.

### High-Level UI State
- **`zustand`**: A minimal global state manager.
- *Why:* Used strictly for high-level, low-frequency UI updates like Score and HUD indicators. 
